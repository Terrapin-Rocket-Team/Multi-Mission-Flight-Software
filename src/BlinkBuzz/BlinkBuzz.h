...
namespace mmfs
{
	...
	public:
		...
		~BlinkBuzz();
		void init(int *allowedPins, int numPins, bool enableAsync = true, int queueChanges = 50);
	...
	private:
		int maxQueueSize = 0; // max number of actions that can be queued (turning on is a separate action from turning off)
		int numPins = 0;
		int *allowedPins = nullptr; // list of pins that are allowed to be used (prevents misfiring pins on accident thru typos)

		// each pin has its own queue of actions
		bool *pinState = nullptr;	// each pin's current on/off state - len: numPins
		bool *pinIsIndef = nullptr; // each pin's q restart spot for indefinite patterns (-1 for no indef state) - len: numPins
		int *pinQStart = nullptr;	// each pin's queue start index (i.e. the next timestamp to be executed) - len: numPins
		int *pinQEnd = nullptr;		// each pin's queue end index (i.e. the last timestamp to be executed) - len: numPins

		int **pinQ = nullptr; // all pins' queue of timestamps - size: numPins x maxQueueSize
